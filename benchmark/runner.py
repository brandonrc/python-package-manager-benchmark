...
def detect_tools() -> dict[str, str | None]:
    ...
    for tool_name in ["pixi", "uv", "conda", "mamba", "pip", "poetry"]:
        path = shutil.which(tool_name) or (shutil.which("pip3") if tool_name == "pip" else None)
        if not path:
...
def benchmark_tests(tool_name: str, config: dict, result: BenchmarkResult, num_runs: int):
    ...
    for i in range(num_runs):
        ...
        # Record timing even on partial failure (some tests pass, some fail)
        # pytest exit code 1 = some tests failed, but the suite ran
        result.test_execution.runs.append(elapsed)
        if success:
            print(f"    Run {i+1}: {elapsed:.1f}s (all passed)")
        else:
            result.notes.append(f"Test run {i+1} had failures: {output[-500:]}")
            print(f"    Run {i+1}: {elapsed:.1f}s (some failures)")
